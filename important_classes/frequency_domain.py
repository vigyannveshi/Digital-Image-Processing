...
class FrequencyDomain:
    ...
    def getW(self,N,dr=10):
        W=np.zeros((N,N),dtype=complex)
        for n in range(0,N):
            for k in range(0,N):
                W[n][k]=np.round(np.exp((-2j*np.pi*n*k)/N),dr)
        return W


    def dft(self,xn,N=None,dr=10):
        if N==None:
            N=len(xn)
        else:
            if len(xn)>=N:
        ...
        return np.dot(xn,self.getW(N,dr))


    def idft(self,Xk,N=None,dr=10):
        if N==None:
            N=len(Xk)
        Wct=np.conjugate(np.transpose(self.getW(N,dr)))
    ...
    def dft2(self,xyn,M=None,N=None,dr=10):
    ...
    def idft2(self,XYK,M=None,N=None,dr=10):
        if M==None and N==None:
            M,N=np.array(XYK).shape
        if M>N:
            Wct_M=np.conjugate(np.transpose(self.getW(M,dr)))
            Wct_N=Wct_M[0:N,0:N]
        else:
            Wct_N=np.conjugate(np.transpose(self.getW(N,dr)))
            Wct_M=Wct_N[0:M,0:M]
